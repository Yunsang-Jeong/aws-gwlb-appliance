...
class IPv4:
    ...
    def __init__(self, raw_packet: bytes):
        ...
        self.header_without_options = bytearray(
            raw_packet[: self.header_size_without_options]
        )
...
class Geneve:
    ...
    def __init__(self, raw_packet: bytes):
        ...
        while option_index < self.options_length * 4:
            self.options.append(
                GeneveOption(
                    raw_packet[self.header_size_without_options + option_index :]
                )
            )
...
while not socket_closer.close_now:
    ...
    for s in rs:
        if s == health_socket:
            ...
            inner_ipv4 = IPv4(
                data[ipv4.header_size + udp.header_size + geneve.header_size :]
            )
            inner_tcp = TCP(
                data[
                    ipv4.header_size
